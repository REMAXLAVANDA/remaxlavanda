// Fırsatlar modülü mock verisi — supabase/migrations/..._init_schema.sql
// içindeki `opportunities` tablosuyla aynı alan adlarını kullanır, böylece
// gerçek Supabase entegrasyonuna geçişte sadece api.js değişecek.

const day = 24 * 60 * 60 * 1000
const daysAgo = (n) => new Date(Date.now() - n * day).toISOString()

// Rol switcher'da olmayan, sadece görünürlük/sahiplik senaryosunu
// göstermek için kullanılan "başka danışman" — RLS'te bu kişinin claim
// ettiği kayıtların, geçerli kullanıcı danışmansa görünmemesi beklenir.
export const OTHER_USERS = {
  'ext-danisman-2': { id: 'ext-danisman-2', name: 'Zeynep Kaya' },
  'ext-danisman-3': { id: 'ext-danisman-3', name: 'Emre Şahin' },
}

export function resolveUserName(id, knownUsers) {
  if (!id) return null
  return knownUsers[id]?.name ?? OTHER_USERS[id]?.name ?? 'Bilinmeyen kullanıcı'
}

export const MOCK_OPPORTUNITIES = [
  {
    id: 'opp-1',
    type: 'satici',
    category: 'konut',
    islemTipi: 'satilik',
    leadAd: 'Mehmet Demir',
    leadTelefon: '0555 111 22 33',
    ozet: "Süleymanpaşa'da 3+1 daire satılık, deniz manzaralı",
    konum: 'Hürriyet Mahallesi',
    odaSayisi: '3+1',
    m2: 140,
    fiyat: 4750000,
    status: 'acik',
    ownerId: 'u-ofis',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(1),
    // Lead Havuzu'ndan dönüştü — bkz. mockLeads.js id:7 (durum: atandi).
    kaynakLeadId: 7,
  },
  {
    id: 'opp-2',
    type: 'alici',
    category: 'ticari',
    islemTipi: 'kiralik',
    leadAd: 'Ayşe Şahin',
    leadTelefon: '0555 222 33 44',
    ozet: "Merkez'de kiraya uygun dükkan arıyor",
    konum: 'Tekirdağ / Merkez',
    fiyat: 25000,
    status: 'claimed',
    ownerId: 'u-ofis',
    claimerId: 'u-danisman',
    claimedAt: daysAgo(2),
    createdAt: daysAgo(4),
  },
  {
    id: 'opp-3',
    type: 'satici',
    category: 'arsa',
    islemTipi: 'satilik',
    leadAd: 'Hasan Öz',
    leadTelefon: '0555 333 44 55',
    ozet: "Çorlu'da 500 m² imarlı arsa",
    konum: 'Tekirdağ / Çorlu',
    fiyat: 1250000,
    status: 'acik',
    ownerId: 'u-owner',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(6),
  },
  {
    id: 'opp-4',
    type: 'alici',
    category: 'konut',
    islemTipi: 'satilik',
    leadAd: 'Fatma Yıldız',
    leadTelefon: '0555 444 55 66',
    ozet: '2+1, bütçe 3.5M TL, merkeze yakın',
    konum: 'Tekirdağ / Merkez',
    fiyat: 3500000,
    status: 'claimed',
    ownerId: 'u-ofis',
    claimerId: 'ext-danisman-2',
    claimedAt: daysAgo(5),
    createdAt: daysAgo(9),
  },
  {
    id: 'opp-5',
    type: 'satici',
    category: 'ticari',
    islemTipi: 'satilik',
    leadAd: 'Kemal Aydın',
    leadTelefon: '0555 555 66 77',
    ozet: 'Depo/atölye olarak kullanılan müstakil yapı — satılık',
    konum: 'Tekirdağ / Çerkezköy',
    fiyat: 2100000,
    status: 'acik',
    ownerId: 'u-broker',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(10),
  },
  {
    id: 'opp-6',
    type: 'alici',
    category: 'konut',
    islemTipi: 'satilik',
    leadAd: 'Tutku Yolaçan Okur',
    leadTelefon: '0555 666 77 88',
    ozet: 'Gecikmiş müşteri takibi — başlangıç süreci eksik',
    konum: 'Tekirdağ / Süleymanpaşa',
    fiyat: 5200000,
    status: 'kapandi',
    ownerId: 'u-ofis',
    claimerId: 'u-danisman',
    claimedAt: daysAgo(20),
    createdAt: daysAgo(25),
  },
  {
    id: 'opp-7',
    type: 'satici',
    category: 'ticari',
    islemTipi: 'satilik',
    leadAd: 'Uğur Parlak',
    leadTelefon: '0555 777 88 99',
    ozet: 'Vazgeçti — ilanı kaldırmak istiyor',
    konum: 'Tekirdağ / Malkara',
    fiyat: 850000,
    status: 'iptal',
    ownerId: 'u-owner',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(35),
  },
  {
    id: 'opp-8',
    type: 'alici',
    category: 'arsa',
    islemTipi: 'satilik',
    leadAd: 'Elif Korkmaz',
    leadTelefon: '0555 888 99 00',
    ozet: 'Bağ evi için 1000+ m² arsa arıyor',
    konum: 'Tekirdağ / Şarköy',
    fiyat: 900000,
    status: 'acik',
    ownerId: 'u-ofis',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(0),
  },
  {
    id: 'opp-9',
    type: 'satici',
    category: 'konut',
    islemTipi: 'satilik',
    leadAd: 'Serkan Bulut',
    leadTelefon: '0555 999 00 11',
    ozet: 'Yeni yapılmış 4+1 villa, satılık',
    konum: 'Tekirdağ / Süleymanpaşa',
    fiyat: 8900000,
    status: 'claimed',
    ownerId: 'u-broker',
    claimerId: 'ext-danisman-2',
    claimedAt: daysAgo(45),
    createdAt: daysAgo(50),
  },
  {
    id: 'opp-10',
    type: 'satici',
    category: 'ticari',
    islemTipi: 'kiralik',
    leadAd: 'Nurcan Aksoy',
    leadTelefon: '0555 101 20 30',
    ozet: "Merkez'deki dükkanını kiraya vermek istiyor",
    konum: 'Tekirdağ / Merkez',
    fiyat: 30000,
    status: 'acik',
    ownerId: 'u-ofis',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(2),
  },
  // Eski "Diğer" kategorisi kaldırıldı (bkz. lib/categories.js) — bu kayıt
  // mock modda "kategorisi belirsiz" bandının görünmesi için bilerek
  // bırakıldı (broker isteği, 2026-09-17).
  {
    id: 'opp-11',
    type: 'alici',
    category: 'diger',
    islemTipi: 'satilik',
    leadAd: 'Tolga Emre',
    leadTelefon: '0555 111 20 30',
    ozet: 'Franchise için uygun yer arıyor — kategori netleşmedi',
    konum: null,
    fiyat: null,
    status: 'acik',
    ownerId: 'u-ofis',
    claimerId: null,
    claimedAt: null,
    createdAt: daysAgo(60),
  },
]
